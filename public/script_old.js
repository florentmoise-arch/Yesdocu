// YesDocu 2.1.0 / WEB 1.0.9 — Routing URLs (onglets + ?p=) + API prefix /api
// - Onglets: /impressions, /reimpressions, /suivi, /admin (+ back/forward)
// - Pagination Impressions reflétée en ?p=<num> (mais jamais ?p=1)
// - Enter = Chercher dans la zone filtres (sauf quand on saisit un tag)
// - Toutes les requêtes fetch pointent vers /api/*
...
const stateImp = {
  ...
  pagination: true,
  suggestions: new Set(),       // suggestions désactivées
  dateSuggestions: new Set(),   // suggestions désactivées
};
...
function setActiveTab(t){
  Object.values(tabs).forEach(x=>{x.tab.classList.remove('active');x.sec.classList.remove('active')});
  t.tab.classList.add('active'); t.sec.classList.add('active');
}
function activeKey(){
  return Object.entries(tabs).find(([k,v])=>v.tab.classList.contains('active'))?.[0];
}
...
function navigateTo(tabKey, { replace=false } = {}){
  const basePath = tabToPath[tabKey] || '/impressions';
  const params = new URLSearchParams(window.location.search);

  // N'affiche ?p=... que si Impressions + pagination ON + page > 1
  if (tabKey === 'imp' && stateImp.pagination && stateImp.page > 1) {
    params.set('p', String(stateImp.page));
  } else {
    params.delete('p');
  }
  ...
  setActiveTab(tabs[tabKey]);
  refreshActiveTab();
}

function updateImpURLPage({ replace=true } = {}){
  const path = tabToPath['imp'];
  const params = new URLSearchParams(window.location.search);

  if (stateImp.pagination && stateImp.page > 1) {
    params.set('p', String(stateImp.page));
  } else {
    params.delete('p');
  }
...
tabs.imp.tab.addEventListener('click', () => navigateTo('imp'));
tabs.reimp.tab.addEventListener('click', () => navigateTo('reimp'));
tabs.suivi.tab.addEventListener('click', () => navigateTo('suivi'));
tabs.admin.tab.addEventListener('click', () => navigateTo('admin'));
...
/* --------------------------------- Utils ---------------------------------- */
function fmtDate(d){
  const date=new Date(d); const pad=n=>String(n).padStart(2,'0');
...
async function loadHotfolders(){
  const r = await fetch(`${API}/hotfolders`); if (!r.ok) return;
...
function syncDocTypeUI(){
  const v=impDocType?.value||'Liste';
  groupImpPrefix.style.display = (v==='CEB') ? 'none' : '';
}
impDocType.addEventListener('change', ()=>{ syncDocTypeUI(); refreshImp(); });
syncDocTypeUI();
...
impPagToggle.addEventListener('change', ()=>{
  stateImp.pagination=!!impPagToggle.checked;
  stateImp.page=1;
  renderImp();
  updateImpURLPage({ replace:true });
});

// Chercher / Reset
document.getElementById('impSearch').addEventListener('click', ()=>{
  stateImp.page=1;
  refreshImp();
  updateImpURLPage({ replace:true });
});
...
document.getElementById('submitImp').addEventListener('click', async ()=>{
  ...
  try{
    const r=await fetch(`${API}/process/impressions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jobs,defaultHotfolder})});
...
async function loadImpressions(){
  const r=await fetch(`${API}/impressions`);
  if(!r.ok){
...
function applyNomenclatureFilters(rows, f){
  ...
  // DocType = CEB -> uniquement CEB ; sinon uniquement la liste Type autorisée
  if (f.docType==='CEB') {
...
function renderImp(){
  ...
  tb.onclick = async (e)=>{
    ...
    if (!defaultHotfolder){ alert("Choisissez une imprimante (lot)"); return; }
    try{
      const r=await fetch(`${API}/process/impressions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jobs:[{name}],defaultHotfolder})});
      const js=await r.json(); if(!r.ok) throw new Error(js.message||'Erreur soumission');
  ...
  renderPagination('pag-imp', stateImp.page, maxPage, (p)=>{ stateImp.page=p; renderImp(); updateImpURLPage({ replace:true }); });
  setupSortHeaders();
}
...
async function loadReimpressions(){
  const r=await fetch(`${API}/reimpressions`); if(!r.ok){ document.getElementById('tb-reimp').innerHTML=''; return; }
  const rows=await r.json();
  ...
  [...tb.querySelectorAll('.reprint-one')].forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      try{
        const r=await fetch(`${API}/process/reimpressions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jobs:[{name,hotfolder:hf}],defaultHotfolder:hf})});
  ...
  document.getElementById('submitReimp')?.addEventListener('click', async ()=>{
    ...
    try{
      const r=await fetch(`${API}/process/reimpressions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ jobs:names.map(n=>({name:n,hotfolder:hf})), defaultHotfolder:hf })});
      const js=await r.json(); if(!r.ok) throw new Error(js.message||'Erreur réimpression');
      await loadReimpressions();
    }catch(e){ alert('Erreur: '+e.message); }
  });
...
async function loadSuivi(){
  const r=await fetch(`${API}/suivi`); if(!r.ok){ document.getElementById('tb-suivi').innerHTML=''; return; }
  const rows=await r.json();
...
document.getElementById('admReset')?.addEventListener('click', async ()=>{
  ...
  const r=await fetch(`${API}/admin/reset-password`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ username, newPassword })});
...
document.getElementById('hfAdd')?.addEventListener('click', async ()=>{
  ...
  const r=await fetch(`${API}/admin/hotfolders`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ name, path })});
  const js=await r.json(); if(!r.ok){ alert(js.message||'Erreur'); return; }
...
/* ---------------------------------- Init ---------------------------------- */
syncFromURL(); // ouvre onglet + page depuis l’URL
refreshMe();
...
router.get('/', requireRole(['admin','responsable']), (req, res) => {
  res.json(DOC.listReimpressions());
});

router.post('/process', requireRole(['admin','responsable']), (req, res) => {
  ...
  for (const j of jobs) {
    ...
    try {
      ...
      DOC.upsert({
        ...
        status: 'en_cours', dest_hotfolder: hf.name, reprint_count: (row.reprint_count||0)+1,
        created_at: row.created_at, updated_at: new Date().toISOString()
      });

      done.push(row.name);
    } catch(e){ console.error('Soumission réimpression échouée:', row.name, e.message); }
  }

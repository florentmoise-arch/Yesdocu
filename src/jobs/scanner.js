...
// - InPrint : contrôle doublons + move -> InProgress (pending)
// - InReprint : PAS de contrôle doublons + move -> InProgress (reprint_pending)
// - Comptage pages robuste (pdf-parse → pdf-lib → regex), jamais d'exception non attrapée
...
function isValidPDF(filePath) {
  try {
    const head = fs.readFileSync(filePath, { encoding: 'latin1' });
...
async function scanInputDir(dir, mode) {
  ...
  let files = [];
  try { files = fs.readdirSync(dir).filter(f => !f.startsWith('~') && isPDFExt(f)); }
  catch { return; }
  ...
  for (const file of files) {
    ...
    // Contrôle de doublons UNIQUEMENT pour 'print'
    ...
    if (mode === 'print') {
      ...
      // InReprint → InProgress (PAS de contrôle doublons)
      const dest = path.join(PATHS.INPROGRESS, file);
      try {
        safeMove(abs, dest);
        let pages = 0;
